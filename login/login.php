...
<?
  ...
  @extract($_SERVER); 
   ...
   $num_match = mysql_num_rows($result);    //  1 아님 0   

   if(!$num_match)    //  검색된 레코드 없을 경우 
   {
     echo("
           <script>
    ...
    else    //해당 아이디가 검색되었으면.. (id + pw)
    {
    		 $row = mysql_fetch_array($result);   // 검색된 레코드를 $row 배열에 담는다  -> $row[id], $row[pw],... $row[level]  array이렇게
