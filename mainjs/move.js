...
$(document).ready(function() {
  ...
  var onoff=true; // true=>타이머 동작중 , false=>동작하지 않을때
     ...
     $('.btn1').css('background','#d00024'); //첫번째 불켜
    $('.btn1').css('width','30');
    
    $('.gallery_text li:eq(0)').fadeIn('normal');
 ...
 function moveg(){
      cnt+=direct;  //카운트가 1 2 3 4 5 4 3 2 1 2 3 4 5 ......
        ...
        $('.gallery_text li:eq('+ (cnt-1) +')').fadeIn('normal');
                                                
	   for(var i=1;i<=imageCount;i++){
          $('.btn'+i).css('background','#fff'); //버튼불다꺼!!
         $('.btn'+i).css('width','16'); 
      }
      $('.btn'+cnt).css('background','#d00024');//자신만 불켜
      $('.btn'+cnt).css('width','30');   
                           
		$('.gallery').animate({left:position}, 'normal'); //겔러리 무비의 left값을 움직여라~
  ...
  $('.mbutton').click(function(event){  //각각의 버튼을 클릭한다면...
    ...
    $('.btn'+cnt).css('background','#d00024');//자신 버튼만 불켜 
    $('.btn'+cnt).css('width','30');
